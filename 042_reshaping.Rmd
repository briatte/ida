...
Exploring this dataset is easy if you need simple measures like raw frequencies. Here, for instance, is the number of different observations in the data for each major party affiliation, and the average DW-NOMINATE score in each group.

```{r dwnominate-table}
# Raw frequencies (N) by party.
table(dwNominate$majorParty)
# Mean DW-NOMINATE score by party.
with(dwNominate, tapply(dwnom1, majorParty, mean))
```

There is more than one way to do [calculations by groups][gs-aggr] in R.
...
```{r dwnominate2, message=FALSE, tidy=FALSE}
library(plyr)
