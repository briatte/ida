...
The `cor()` function computes Pearson's correlation coefficient. The result will vary between $-1$ (perfect negative correlation) and $+1$ (perfect positive correlation). We use the `with()` function to tell R that both variables are in the same `oecd` dataset. The result indicates a moderate correlation, as previously observed.
