...
This section introduces data import and export (or "I/O", for "input/output") with R. covers a few ways to read/write `CSV` and `TXT` data, as well as common proprietary formats.
...
We now need to get the data from that address, using the `RCurl` library to fetch the online spreadsheet from the `dk.link` object in which we stored the link. Note that the `getURL` command is in a conditional statement that avoids downloading the same file again and again if you already have it.
...
> __Next___: [Reshaping](42_reshaping.html).
