...
In [his code][mages1], Markus Gessman uses a log-linear model to predict the 2012 sprint result from the series of past results, using all data from 1900 to today. To understand [this model][mages2], we take a quick look at the sprinter results and add a LOESS curve to the plot, which provides a smoothed trend of the series:

[mages1]: http://lamages.blogspot.fr/2012/08/london-olympics-100m-mens-sprint-results.html
[mages2]: http://lamages.blogspot.co.uk/2012/07/london-olympics-and-prediction-for-100m.html
...
The log-linear prediction has added three columns of data (the prediction with its lower and upper bounds) and a few rows of data for several years, including year 2012. The graph below singles out that prediction, at `r olympics$fit[31]` seconds, and shows the rest of the predicted trend.
...
As Markus Gessman [notes][mages2], his model was very close to reality: in 2012, the best sprinter in the 100m men's run was only slightly quicker than predicted, at 9.63 seconds. This illustrates the common use of models to forecast results from data like financial markets, [elections][538] or sports events. Note that the model actually improves if you take year 1896 into account:
