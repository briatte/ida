...
```{r dw-quantile-function, tidy = FALSE}
...
The first map shows the swing in vote share, and the next ones show the actual vote shares. 

```{r dw-map-choropleth-auto, fig.width = 12, fig.height = 6.6}
# Choropleth map for Obama swing vote.
g + aes(fill = quantize(Swing, q = 5)) + 
  ggtitle("Swing (2012 - 2008) Obama vote share\n")
# Choropleth map for Obama vote share, 2008.
g + aes(fill = quantize(Obama08)) + 
  ggtitle("Obama vote share, 2008\n")
# Choropleth map for Obama vote share, 2012.
g + aes(fill = quantize(Obama12)) + 
  ggtitle("Obama vote share, 2012\n")
```
...
[ds-simplemaps]: http://is-r.tumblr.com/post/37708137014/us-state-maps-using-map-data
[ds-choropleth]: https://dsparks.wordpress.com/2011/02/21/choropleth-tutorial-and-regression-coefficient-plots/
[ds-isarithmic]: https://dsparks.wordpress.com/2012/07/18/mapping-public-opinion-a-tutorial/
