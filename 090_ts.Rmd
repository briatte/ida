...
This process works like the `as.Date()` base function shown in [Session 4.2](042_reshaping.html), and for which Teetor, ch. 7.9-7.11, is a good starting point. The `lubridate` package is a convenience tool that deals with date formats in a more flexible way. Extracting the year from the `Date` variable created above, for instance, can be done effortlessly, just as any other form of date extraction:
