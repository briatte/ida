...
Because loops proceed by sequential exhaustion ("do that to $x$", "do it again to $x+1$", "do it again…"), they can become computationally intensive when they iterate through large arrays of values. At that stage, your code needs to be optimized and you might need to use several computer processors to [parallelize the execution][cran-foreach] of the loop.
...
The group of observations is the vector `r 1:3`. This is made possible by the support for vector arguments in the square function. Many functions accept vectors as arguments, which makes it possible to compute things efficiently, as in this example of an [explosive roommate combinations][fs-expl] function, coded by Francis Smart:
...
This function looks at the number of potential conflicts that might arise when "each roommate pair has the possibility of severely bothering another roommate". The code calculates that value from the combination equation, $\frac{n!}{k!(n-k)!}$, where $n$ is the number of roommates and $k = 1$ the number of conflicts.

This function can be calculated for an increasing number of roommates. To plot that part of the function, you then need to create a vector of values and apply the explosive roommates combination to it. This is done below by writing up the function into a `sapply` command that goes through the vector of values `x` and returns a vector of results `y`:
