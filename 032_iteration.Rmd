...
Vectorization is a way to replace (most) loops with a more efficient way to handle the data. R contains a lot of vectorized functions that will run [significantly quicker](http://quanttrader.info/public/FasterRCode.pdf) than memory-intensive loops of code. The basic idea is to apply a function to a group of observations rather than telling R to apply that function to $i = { 1, 2, ... n}$, as here:
