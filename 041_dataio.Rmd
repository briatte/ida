...
Here's the code to download a dataset that we will use in future classes. Install the `downloader` package before running the code (by now, you should know how to install a package).

```{r QOG-Basic-data, results='hide'}
require(downloader)
...
QOG datasets feature a wide range of variables from various sources, including UN data, World Development Indicators and several research datasets. The example below uses data from the Barro and Lee and Maddison studies. Another advantage of Quality of Government data is that it comes with several country identifiers that allow to match external data to it, as we do below by adding continental origin based on ISO-3C country codes.
