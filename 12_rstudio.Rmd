...
Use [this link](http://www.rstudio.com/ide/download/desktop) to download RStudio. Installation should then be pretty straightforward.

We previously mentioned [Gordon Davis' video tutorial](https://www.youtube.com/watch?v=WJDrYUqNrHg&list=PL8BE0E317807A9A21) for Mac OS X users. It shows how to install R and RStudio, and also showcases some of its customizable features, starting at timestamp 6:18. Give it a try, regardless of your operating system:

<iframe width="800" height="450" src="https://www.youtube.com/embed/WJDrYUqNrHg#t=6m18s" frameborder="0" allowfullscreen></iframe>
...
Now copy-paste [this code](https://gist.github.com/4555856) into the blank R script, select it all, and run. You will see results get printed out (in black) in the Console, along with the code (in blue). Objects will also appear in the Workspace, and a help page will open in the bottom-right quadrant.
...
One last functionality that we want to show as part of the RStudio interface has to do with plots. Let's take inspiration from a [basic ggplot2 tutorial](http://blog.echen.me/2012/01/17/quick-introduction-to-ggplot2/) (`ggplot2` is a graphics package that you might have [previously](11_r.html) installed when trying out package installation in R).

```{r ggplot2-1, eval=FALSE}
# In case you have not yet installed ggplot2, run this.
install.packages("ggplot2")
```

We are going to take a quick look at some example data that is part of the R software. The data describes a set of fifty flowers studied by famous statistician Ronald Fisher in an article published in 1936. The data itself were collected by Edgar Anderson and published one year before.

```{r ggplot2-2}
# Load ggplot2 package.
library(ggplot2)
# Look at a default dataset.
head(iris)
# Quick plot.
